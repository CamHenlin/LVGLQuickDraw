 ...
 *  STATIC PROTOTYPES
 **********************/
static Boolean obj_valid_child(const lv_obj_t * parent, const  lv_obj_t * obj_to_find);
...
Boolean lv_debug_check_null(const void * p)
{
    if(p) return true;
...
Boolean lv_debug_check_mem_integrity(void)
{
    return lv_mem_test() == LV_RES_OK ? true : false;
}

Boolean lv_debug_check_obj_type(const lv_obj_t * obj, const char * obj_type)
{
...
Boolean lv_debug_check_obj_valid(const lv_obj_t * obj)
{
    ...
    while(disp) {
        ...
        _LV_LL_READ(disp->scr_ll, scr) {
            ...
            Boolean found = obj_valid_child(scr, obj);
...
Boolean lv_debug_check_style(const lv_style_t * style)
{
...
Boolean lv_debug_check_style_list(const lv_style_list_t * list)
{
...
Boolean lv_debug_check_str(const void * str)
{
...
static Boolean obj_valid_child(const lv_obj_t * parent, const lv_obj_t * obj_to_find)
{
    ...
    _LV_LL_READ(parent->child_ll, child) {
        ...
        /*Check the children*/
        Boolean found = obj_valid_child(child, obj_to_find);
        if(found) return true;
    }
