...
static inline Boolean is_tick_with_label(uint8_t tick_num, lv_chart_axis_cfg_t * axis);
...
static void draw_series_line(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * clip_area)
{
    ...
    Boolean mask_ret = _lv_area_intersect(&series_mask, series_area, clip_area);
    ...
    Boolean has_area = lv_obj_get_style_bg_opa(chart, LV_CHART_PART_SERIES) > LV_OPA_MIN ? true : false;
    Boolean has_fade = false;
    if(has_area) {
