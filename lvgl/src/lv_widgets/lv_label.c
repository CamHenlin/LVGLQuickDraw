...
static Boolean lv_label_set_dot_tmp(lv_obj_t * label, char * data, uint16_t len);
 ...
 */
void lv_label_set_recolor(lv_obj_t * label, Boolean en)
{
    LV_ASSERT_OBJ(label, LV_OBJX_NAME);
 ...
 */
Boolean lv_label_get_recolor(const lv_obj_t * label)
{
    LV_ASSERT_OBJ(label, LV_OBJX_NAME);
 ...
 */
Boolean lv_label_is_char_under_pos(const lv_obj_t * label, lv_point_t * pos)
{
    LV_ASSERT_OBJ(label, LV_OBJX_NAME);
    LV_ASSERT_NULL(pos);
...
static lv_design_res_t lv_label_design(lv_obj_t * label, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK)
        ...
        lv_area_t txt_clip;
        Boolean is_common = _lv_area_intersect(&txt_clip, clip_area, &txt_coords);
