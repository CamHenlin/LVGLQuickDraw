...
LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t * map_area, const lv_area_t * clip_area,
                                              const uint8_t * map_p,
                                              lv_draw_img_dsc_t * draw_dsc,
                                              Boolean chroma_key, Boolean alpha_byte);
 ...
 */
Boolean lv_img_cf_is_chroma_keyed(lv_img_cf_t cf)
{
    Boolean is_chroma_keyed = false;
 ...
 */
Boolean lv_img_cf_has_alpha(lv_img_cf_t cf)
{
                                                       ...
                                                       lv_draw_img_dsc_t * draw_dsc)
{
    ...
    Boolean chroma_keyed = lv_img_cf_is_chroma_keyed(cdsc->dec_dsc.header.cf);
    Boolean alpha_byte   = lv_img_cf_has_alpha(cdsc->dec_dsc.header.cf);
...
LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t * map_area, const lv_area_t * clip_area,
                                              const uint8_t * map_p,
                                              lv_draw_img_dsc_t * draw_dsc, Boolean chroma_key, Boolean alpha_byte)
{
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        Boolean transform = draw_dsc->angle != 0 || draw_dsc->zoom != LV_IMG_ZOOM_NONE ? true : false;
#else
